...
namespace HttpResponseApp
{
    public partial class Form1 : Form
    {
        ...
        private async void btnStartAnalyze_Click(object sender, EventArgs e)
        {
            ...
            stop = false;
            checkBoxSingleUrlLighthouse.Enabled = false;
            linkLabelClearResult.Enabled = false;
            btnStartAnalyze.Enabled = false;

            string url = txtSingleUrl.Text;
            ...
            using (var httpClient = new HttpClient())
            {
                try
                {
                    ...
                    if (checkBoxSingleUrlLighthouse.Checked)
                    {
                        analiyticsResult = await GetPerformanceResult(url);
                    }
                    else
                    ...
                    if (MessageBox.Show("Ýþlem tamamlandý.", "Stop", MessageBoxButtons.OK, MessageBoxIcon.Asterisk) == DialogResult.OK)
                    {
                        stop = true;
                        checkBoxSingleUrlLighthouse.Enabled = true;
                        linkLabelClearResult.Enabled = true;
        ...
        private async void btnStopAnalyze_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Ýþlemi durdurmak istediðinizden emin misiniz?", "Stop", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                stop = true;
        ...
        private void linkLabelClearResult_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            dataGridView2.Rows.Clear();
        }
